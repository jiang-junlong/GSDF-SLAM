                                                                 ...
                                                                 size_t N)
{
  ...
  cub::DeviceScan::InclusiveSum(nullptr, img.bucket_count_scan_size,
                                img.bucket_count, img.bucket_count, N);
    ...
    bool debug)
{
  ...
  // std::cout << "forward: focal_x = " << focal_x << ", focal_y = " << focal_y << std::endl;
  ...
  CHECK_CUDA(FORWARD::preprocess(
             ...
             debug)
  // std::cout << "forward: preprocessed! Total gaussians: " << P << std::endl;

  // Debug
  // for (int i = 0; i < P; i++)
  // {
  //   int tiles;
  //   if(cudaMemcpy(&tiles, &geomState.tiles_touched[i], sizeof(int), cudaMemcpyDeviceToHost) == cudaSuccess) {
  //     std::cout << "Gaussian " << i << " touches " << tiles << " tiles" << std::endl;
  //   }
  // }
  ...
  CHECK_CUDA(cub::DeviceScan::InclusiveSum(imgState.bucket_count_scanning_space,
                                           ...
                                           imgState.bucket_offsets, num_tiles),
             debug)
