...
void GaussianMapper::run()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (int i = 0; i < 20; ++i)
        {
            trainForOneIteration_ours();
        }
    }
}

void GaussianMapper::trainForOneIteration_ours()
{
    ...
    gaussians_->setShDegree(model_params_.sh_degree_);
...
/**
 * @brief The training iteration body，后面校验下是否发生变动
 *
 */
