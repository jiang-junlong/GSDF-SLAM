...
int main(int argc, char** argv)
{
    ...
    rs2_intrinsics intrinsics_cam = cam_stream.as<rs2::video_stream_profile>().get_intrinsics();  // 获取相机内参
    ...
    std::shared_ptr<ORB_SLAM3::System> pSLAM = std::make_shared<ORB_SLAM3::System>(argv[1], argv[2], ORB_SLAM3::System::RGBD, 0, "realsense_rgbd");
    ...
    while (!pSLAM->isShutDown())
    {
        ...
        pSLAM->TrackRGBD(im, depth, timestamp, std::vector<ORB_SLAM3::IMU::Point>(), std::to_string(timestamp));  // ORBSLAM系统进行跟踪
    ...
    // Stop all threads
    pSLAM->Shutdown();
    training_thd.join(); // .join阻塞主线程的执行，直到 training_thd 线程结束
    if (use_viewer)
        viewer_thd.join();  // .join阻塞主线程的执行，直到 viewer_thd 线程结束
